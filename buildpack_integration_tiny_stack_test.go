...
func testBuildpackIntegrationTinyStack(t *testing.T, context spec.G, it spec.S) {
	...
	it.Before(func() {
		...
		buildImageID = fmt.Sprintf("%s/noble-base-stack-build-%s", RegistryUrl, uuid.NewString())
		runImageID = fmt.Sprintf("%s/noble-tiny-stack-run-%s", RegistryUrl, uuid.NewString())
...
`,
		...
		Expect(archiveToDaemon(baseStack.BuildArchive, buildImageID)).To(Succeed())
		Expect(archiveToDaemon(tinyStack.RunArchive, runImageID)).To(Succeed())
