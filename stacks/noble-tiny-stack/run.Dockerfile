...
RUN echo "$sources" > /etc/apt/sources.list.d/ubuntu.sources && \
  ...
  rm /etc/os-release && cat /usr/lib/os-release | \
    sed -e 's#PRETTY_NAME=.*#PRETTY_NAME="Paketo Buildpacks Full Noble"#' \
        -e 's#HOME_URL=.*#HOME_URL="https://github.com/paketo-buildpacks/noble-full-stack"#' \
        -e 's#SUPPORT_URL=.*#SUPPORT_URL="https://github.com/paketo-buildpacks/noble-full-stack/blob/main/README.md"#' \
        -e 's#BUG_REPORT_URL=.*#BUG_REPORT_URL="https://github.com/paketo-buildpacks/noble-full-stack/issues/new"#' \
